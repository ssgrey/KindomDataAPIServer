...
namespace KindomDataAPIServer.Common
{
    ...
    public sealed class AdaptiveDataTypeSettings
    {
        ...
        public static AdaptiveDataTypeSettings CreateWellLog()
        {
            return new AdaptiveDataTypeSettings
            {
                ...
                Upload = new AdaptiveUploadSettings { InitialPayloadMiB = 4, MaxPayloadMiB = 28, Concurrency = 5, StableEvaluationRequests = 10, MaxSampleCount = 5000000, MinimumCurveCount = 2 }
            };
        }
    ...
    public sealed class AdaptiveUploadSettings
    {
        ...
        public int MinimumCurveCount { get; set; } = 2;

        [JsonProperty("initialCurveCount")]
        private int LegacyInitialCurveCount
        {
            set { MinimumCurveCount = value; }
        }
    }
    ...
    public sealed class AdaptiveSyncController
    {
        ...
        private const int CurrentWellLogLearningVersion = 4;
        ...
        private readonly bool _isWellLog;
        ...
        public int MinimumCurveCount => _minimumCurveCount;
            ...
            int itemCount = 0)
        {
            try
            {
                lock (_syncRoot)
                {
                    ...
                    bool payloadUtilizationOutsideLearningRange = _isWellLog
                        ? payloadBytes < requestPayloadTargetBytes / 2
                        : payloadBytes > requestPayloadTargetBytes || payloadBytes < requestPayloadTargetBytes / 2;
                    if (payloadUtilizationOutsideLearningRange)
                    {
                        _payloadLearningExcludedRequestCount++;
                        SyncTaskReportService.Instance.Log(
        ...
        public void RecordRequestTooLarge(long failedPayloadBytes)
        {
            lock (_syncRoot)
            {
                ...
                SyncTaskReportService.Instance.LogSummary(
                    $"Adaptive sync {_dataType} request-too-large cycle entered. Failed payload bytes:{failedPayloadBytes}, payload target old/new:{oldPayload}/{_currentPayloadBytes}. Concurrent 413 responses do not stack and upload concurrency is unchanged.");
            }
        }
        ...
        internal AdaptiveDataTypeState CreateLearnedState(string environmentId)
        {
            lock (_syncRoot)
            {
                ...
                {
                    PayloadLearningVersion = _isWellLog ? CurrentWellLogLearningVersion : CurrentPayloadLearningVersion,
                    BestStablePayloadBytes = _bestStableSamplePayloadBytes,
                    BestStableCurveCount = 0,
        ...
        private void ApplyTransportProtection(string signal)
        {
            ...
            {
                SyncTaskReportService.Instance.LogSummary(
                    $"Adaptive sync {_dataType} transport protection entered. Signal:{signal}, concurrency:{_effectiveConcurrency}, payload bytes:{_currentPayloadBytes}; payload target unchanged.");
            }
        }
    ...
    public static class AdaptiveSyncService
    {
        ...
        private static void NormalizeType(string name, AdaptiveDataTypeSettings settings, int absoluteReadMax, int absoluteConcurrencyMax, int absoluteBusinessMax)
        {
            ...
            settings.Upload.MinimumCurveCount = ClampAndLog($"dataTypes.{name}.upload.minimumCurveCount", settings.Upload.MinimumCurveCount, 1, 100000);
        }
